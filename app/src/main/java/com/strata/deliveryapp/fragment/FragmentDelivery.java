...
@SuppressLint("NewApi")
public class FragmentDelivery extends ListFragment{
	...
	public void onListItemClick(ListView l, final View view, final int position, long id) {
		if(!selected_array.get(position).getSelect().equals("true")){
	        ...
	        alert.setMessage("Please confirm the book no: "+book_no+" before DELIVERY");
	        alert.setButton("Yes", new DialogInterface.OnClickListener() {
	           public void onClick(DialogInterface dialog, int which) {
	        	   //String url = "http://"+Config.SERVER_BASE_URL+"/api/v1/orders/delivery.json?api_key="+auth+"&phone="+numb+"&book_no="+book_no+"&membership_no="+memb;
	        	   ...
	        	   String url = "http://"+Config.SERVER_BASE_URL+"/api/delivery_v1/delivery_order/pickup.json?" +
		        	   		...
		        	   		"&delivery_order_id="+order_no+"&branch_id="+SharedValue.data().branch_id+
	...
	public class Member_Custom_Adapter extends ArrayAdapter<Member_Sub_Order>{
		...
		public View getView(int position, View view, ViewGroup parent) {
			...
			if (order.getOrder_type().equals("DeliveryOrder")){
				//MainLL.setBackgroundColor(R.color.light_violet);0xff000000
				MainLL.setBackgroundColor(0xffE3C8F5);
			}
			if (selected_array.get(position).getSelect().equals("true")){
